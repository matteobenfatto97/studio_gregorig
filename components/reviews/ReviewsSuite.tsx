...
export function ReviewCard({ review }: { review: Review }) {
  ...
  // Tilt: 0..1 -> gradi
  const rotateX = useTransform(y, (v) => (v - 0.5) * 6);
  const rotateY = useTransform(x, (v) => (0.5 - v) * 6);

  // Posizione del glow (CSS vars)
  const mx = useTransform(x, (v) => `${Math.round(v * 100)}%`);
  const my = useTransform(y, (v) => `${Math.round(v * 100)}%`);
